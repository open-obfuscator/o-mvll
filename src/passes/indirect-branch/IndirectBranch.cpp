...
bool IndirectBranch::process(Function &F, const DataLayout &DL,
  ...
  for (Instruction *TI : TerminatorsToReplace) {
    ...
    auto CreateGEPFor = [&](BasicBlock *Dest) -> Value * {
    ...
    if (auto *BI = dyn_cast<BranchInst>(TI)) {
      if (BI->isConditional())
        LoadFrom = Builder.CreateSelect(BI->getCondition(),
                                        CreateGEPFor(BI->getSuccessor(0)),
                                        CreateGEPFor(BI->getSuccessor(1)));
      else
        LoadFrom = CreateGEPFor(BI->getSuccessor(0));
    } else if (auto *SI = dyn_cast<SwitchInst>(TI)) {
      LoadFrom = CreateGEPFor(SI->getDefaultDest());

      for (const auto &Case : SI->cases()) {
        Value *Cond =
            Builder.CreateICmpEQ(SI->getCondition(), Case.getCaseValue());
        LoadFrom = Builder.CreateSelect(
            Cond, CreateGEPFor(Case.getCaseSuccessor()), LoadFrom);
      }
    }
...
PreservedAnalyses IndirectBranch::run(Module &M, ModuleAnalysisManager &FAM) {
