...
//     RUN: env OMVLL_CONFIG=%S/config_remove.py clang++ -target aarch64-linux-android -fpass-plugin=%libOMVLL -O1 -c %s \
...
//     RUN: env OMVLL_CONFIG=%S/config_replace.py clang++ -target aarch64-linux-android -fpass-plugin=%libOMVLL -O1 -c %s \
